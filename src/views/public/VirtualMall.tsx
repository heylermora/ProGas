        ...
        <Box display="block">
          <Box position="relative" w="100%" h={{ base: '460px', md: '590px', lg: '620px' }} borderRadius="24px" overflow="hidden" bg="radial-gradient(circle at 50% 50%, #4338CA 0%, #1E1B4B 35%, #0B1026 74%)" _before={{ content: '""', position: 'absolute', inset: 0, opacity: .8, bgImage: 'radial-gradient(circle at 12% 20%, #ffffff 0 1px, transparent 2px), radial-gradient(circle at 70% 35%, #ffffff 0 1px, transparent 2px), radial-gradient(circle at 42% 88%, #ffffff 0 1.5px, transparent 2.5px)', bgSize: '42px 42px, 58px 58px, 73px 73px' }}>
            ...
            {BUSINESS_CATEGORIES.map((category, index) => {
              ...
              return <Button key={category} position="absolute" left={`${left}%`} top={`${top}%`} transform="translate(-50%, -50%)" w={{ base: '48px', sm: '72px', md: '138px' }} minW={{ base: '48px', sm: '72px', md: '138px' }} minH={{ base: '48px', sm: '62px', md: '96px' }} whiteSpace="normal" py={{ base: 1, md: 2 }} px={{ base: 1, md: 2 }} variant="unstyled" bg={selected ? 'brand.500' : 'whiteAlpha.900'} color={selected ? 'white' : 'navy.700'} border={{ base: '2px solid', md: '3px solid' }} borderColor={selected ? 'brand.300' : 'white'} borderRadius={{ base: 'full', md: '18px' }} boxShadow={selected ? { base: '0 0 18px rgba(103, 232, 249, .9)', md: '0 15px 0 #1A5D98, 0 22px 30px rgba(15, 23, 42, .28)' } : { base: '0 0 12px rgba(255,255,255,.55)', md: '0 8px 0 #CBD5E0, 0 13px 22px rgba(15, 23, 42, .20)' }} onClick={() => setSelectedCategory(category)} display="flex" flexDirection="column" justifyContent="center" gap="2px" transition="all .35s cubic-bezier(.2,.8,.2,1)" zIndex={2} _hover={{ transform: 'translate(-50%, calc(-50% - 5px)) scale(1.08)' }}><Text fontSize={{ base: '20px', md: '26px' }}>{categoryEmoji[index]}</Text><Text display={{ base: 'none', md: 'block' }} fontSize="xs" fontWeight="900" noOfLines={2}>{category}</Text><Badge display={{ base: 'none', md: 'inline-flex' }} fontSize="9px" colorScheme={selected ? 'whiteAlpha' : 'brand'}>{total} negocios</Badge>{total > 0 && <Box position="absolute" top={{ base: '1px', md: '7px' }} right={{ base: '1px', md: '7px' }} w={{ base: '7px', md: '9px' }} h={{ base: '7px', md: '9px' }} borderRadius="full" bg="green.400" boxShadow="0 0 0 3px rgba(255,255,255,.72)" />}</Button>;
            })}
