  ...
  return (
    <PublicPage maxW="1280px">
      ...
      <Box bg={panelBg} borderRadius={{ base: '20px', md: '28px' }} p={{ base: '12px', md: '22px' }} boxShadow="lg" mb="22px" overflow="hidden">
        <Flex align="center" justify="space-between" mb="14px" gap="12px" direction={{ base: 'column', sm: 'row' }}><Box><Heading fontSize={{ base: 'lg', md: 'xl' }}>Mapa galáctico</Heading><Text color={muted} fontSize="sm">Tocá una estación para mover al astronauta. Las líneas conectan todas las categorías.</Text></Box><Button size="sm" variant={selectedCategory ? 'outline' : 'solid'} colorScheme="brand" onClick={() => setSelectedCategory('')}>Volver a la plaza</Button></Flex>
        <Box display="block">
          <Box position="relative" w="100%" h={{ base: '460px', md: '590px', lg: '620px' }} borderRadius="24px" overflow="hidden" bg="radial-gradient(circle at 50% 50%, #4338CA 0%, #1E1B4B 35%, #0B1026 74%)" _before={{ content: '""', position: 'absolute', inset: 0, opacity: .8, bgImage: 'radial-gradient(circle at 12% 20%, #ffffff 0 1px, transparent 2px), radial-gradient(circle at 70% 35%, #ffffff 0 1px, transparent 2px), radial-gradient(circle at 42% 88%, #ffffff 0 1.5px, transparent 2.5px)', bgSize: '42px 42px, 58px 58px, 73px 73px' }}>
            ...
            <Box position="absolute" left="39%" top="39%" w="22%" h="22%" borderRadius="full" bg="purple.300" border={{ base: '4px solid', md: '8px solid' }} borderColor="yellow.300" boxShadow="0 0 30px rgba(250, 204, 21, .55), inset 0 0 0 6px rgba(255,255,255,.25)" display="flex" alignItems="center" justifyContent="center"><Stack align="center" spacing="0"><Text fontSize={{ base: '20px', md: '32px' }}>🪐</Text><Text display={{ base: 'none', sm: 'block' }} fontSize="xs" fontWeight="900" color="white">PLAZA CENTRAL</Text></Stack></Box>
            {BUSINESS_CATEGORIES.map((category, index) => {
              ...
              return <Tooltip key={category} label={`${category} · ${total} negocios`} hasArrow placement="top"><Button aria-label={`Ir a ${category}, ${total} negocios`} position="absolute" left={`${left}%`} top={`${top}%`} transform="translate(-50%, -50%)" w={{ base: '42px', md: '60px', lg: '72px' }} h={{ base: '42px', md: '60px', lg: '72px' }} minW={{ base: '42px', md: '60px', lg: '72px' }} minH={{ base: '42px', md: '60px', lg: '72px' }} p="0" variant="unstyled" bg={selected ? 'brand.500' : 'whiteAlpha.900'} color={selected ? 'white' : 'navy.700'} border="3px solid" borderColor={selected ? 'cyan.200' : 'white'} borderRadius="full" boxShadow={selected ? '0 0 0 5px rgba(103, 232, 249, .24), 0 0 22px rgba(103, 232, 249, .95)' : '0 0 12px rgba(255,255,255,.55)'} onClick={() => setSelectedCategory(category)} display="flex" alignItems="center" justifyContent="center" transition="all .28s cubic-bezier(.2,.8,.2,1)" zIndex={2} _hover={{ transform: 'translate(-50%, -50%) scale(1.16)', boxShadow: '0 0 0 6px rgba(103, 232, 249, .30), 0 0 26px rgba(103, 232, 249, 1)' }}><Text fontSize={{ base: '19px', md: '28px', lg: '32px' }}>{categoryEmoji[index]}</Text>{total > 0 && <Box position="absolute" top="2px" right="2px" w={{ base: '7px', md: '9px' }} h={{ base: '7px', md: '9px' }} borderRadius="full" bg="green.400" boxShadow="0 0 0 3px rgba(255,255,255,.72)" />}</Button></Tooltip>;
            })}
            <Box position="absolute" left={`${playerLeft}%`} top={`${playerTop}%`} zIndex={4} transition="left .55s cubic-bezier(.2,.8,.2,1), top .55s cubic-bezier(.2,.8,.2,1)" pointerEvents="none"><Badge position="absolute" left="50%" top={{ base: '-20px', md: '-28px' }} transform="translateX(-50%)" whiteSpace="nowrap" colorScheme="red" borderRadius="full" boxShadow="sm" fontSize={{ base: '8px', md: 'xs' }}><Icon as={MdMyLocation} mr="3px" />ESTÁS AQUÍ</Badge><Box position="absolute" top={{ base: '17px', md: '24px' }} left="50%" w={{ base: '24px', md: '36px' }} h={{ base: '7px', md: '10px' }} borderRadius="full" bg="blackAlpha.400" animation={`${playerShadow} 1s ease-in-out infinite`} /><Box animation={`${playerPulse} 1s ease-in-out infinite`} fontSize={{ base: '28px', md: '42px' }} lineHeight="1">🧑‍🚀</Box></Box>
            <Stack position="absolute" left="4%" bottom="4%" spacing="1" p={{ base: '7px 10px', md: '9px 13px' }} borderRadius="14px" bg="blackAlpha.700" color="white" maxW={{ base: '160px', md: '260px' }}><Text fontSize="9px" letterSpacing=".08em" color="cyan.200" fontWeight="900">DESTINO ACTUAL</Text><Text fontSize={{ base: 'xs', md: 'sm' }} fontWeight="800" noOfLines={1}>{selectedCategory || 'Plaza central'}</Text><Text fontSize="10px" color="whiteAlpha.800">Tocá otra estación para viajar.</Text></Stack>
            <Box display={{ base: 'none', sm: 'block' }} position="absolute" right="4%" bottom="4%" px="12px" py="7px" borderRadius="full" bg="gray.800" color="white" fontSize="xs" fontWeight="800">🚪 SALIDA DEL MAPA</Box>
          </Box>
        </Box>
