...
const mapPositions = [
  [10, 16], [28, 10], [48, 14], [68, 10], [88, 18],
  [13, 36], [32, 32], [68, 32], [87, 38],
  [11, 58], [31, 63], [69, 62], [89, 57],
  [14, 79], [34, 84], [52, 78], [70, 84], [88, 79],
  [24, 48], [50, 50], [76, 48],
];
const businessPositions = [
  [18, 28], [38, 20], [62, 22], [82, 30], [74, 55], [57, 73], [35, 72], [20, 55],
];
...
export default function VirtualMall() {
  ...
  const mapPath = [0, 1, 2, 3, 4, 8, 12, 17, 16, 15, 14, 13, 9, 5, 6, 18, 19, 20, 7, 11, 10]
    ...
    <PublicPage maxW="1280px">
      <Stack spacing={{ base: '18px', md: '22px' }}>
        ...
        <Box bg={panelBg} borderRadius={{ base: '22px', md: '30px' }} p={{ base: '12px', md: '22px' }} boxShadow="xl" overflow="hidden">
          ...
          <Box display="grid" gridTemplateColumns={{ base: '1fr', xl: 'minmax(0, 1.45fr) minmax(320px, .55fr)' }} gap="16px" alignItems="stretch">
            <Box position="relative" minH={{ base: '560px', md: '640px', xl: '680px' }} borderRadius="26px" overflow="hidden" bg="radial-gradient(circle at 50% 50%, #4338CA 0%, #1E1B4B 37%, #070B1F 78%)" _before={{ content: '""', position: 'absolute', inset: 0, opacity: .78, bgImage: 'radial-gradient(circle at 12% 20%, #ffffff 0 1px, transparent 2px), radial-gradient(circle at 70% 35%, #ffffff 0 1px, transparent 2px), radial-gradient(circle at 42% 88%, #ffffff 0 1.5px, transparent 2.5px)', bgSize: '42px 42px, 58px 58px, 73px 73px' }}>
              ...
              <Box position="absolute" left="4%" right="4%" top="47%" h={{ base: '34px', md: '58px' }} bg="cyan.300" opacity=".14" borderY={{ base: '4px solid', md: '8px solid' }} borderColor="cyan.100" transform="rotate(-4deg)" />
              <Box position="absolute" top="6%" bottom="6%" left="47%" w={{ base: '34px', md: '58px' }} bg="cyan.300" opacity=".14" borderX={{ base: '4px solid', md: '8px solid' }} borderColor="cyan.100" transform="rotate(5deg)" />

              <Box as="button" type="button" aria-label="Ver todas las zonas" onClick={() => selectCategory('')} position="absolute" left="50%" top="50%" transform="translate(-50%, -50%)" w={{ base: '118px', md: '164px' }} h={{ base: '118px', md: '164px' }} borderRadius="full" bg="rgba(250, 204, 21, .82)" border={{ base: '5px solid', md: '9px solid' }} borderColor="yellow.200" boxShadow="0 0 34px rgba(250, 204, 21, .55), inset 0 0 0 8px rgba(255,255,255,.22)" display="flex" alignItems="center" justifyContent="center" cursor="pointer" zIndex={1} _hover={{ filter: 'brightness(1.08)' }} _focusVisible={{ outline: '3px solid', outlineColor: 'cyan.200', outlineOffset: '4px' }}>
              ...
              {selectedCategory && shownBusinesses.slice(0, 8).map((business, index) => {
                const [left, top] = businessPositions[index];
                const selected = selectedBusiness?.id === business.id;
                return (
                  <Button key={business.id} aria-label={`Ver ${business.name}`} onClick={() => setSelectedBusinessId(business.id)} position="absolute" left={`${left}%`} top={`${top}%`} transform="translate(-50%, -50%)" w={{ base: '74px', md: '96px' }} minW={{ base: '74px', md: '96px' }} h={{ base: '54px', md: '66px' }} variant="unstyled" borderRadius="18px" bg={selected ? 'white' : 'whiteAlpha.900'} color="navy.800" border="2px solid" borderColor={selected ? 'yellow.300' : 'whiteAlpha.800'} boxShadow={selected ? '0 0 0 4px rgba(250,204,21,.24), 0 16px 26px rgba(0,0,0,.25)' : '0 10px 20px rgba(0,0,0,.2)'} zIndex={3} display="flex" flexDirection="column" alignItems="center" justifyContent="center" gap="2px" _hover={{ transform: 'translate(-50%, -50%) scale(1.06)' }}>
                    {business.logoUrl ? <Image src={business.logoUrl} alt={business.name} maxW="42px" maxH="24px" objectFit="contain" /> : <Icon as={MdStorefront} w="24px" h="24px" color="brand.500" />}
                    <Text maxW="92%" noOfLines={1} fontSize={{ base: '9px', md: '10px' }} fontWeight="900">{business.name}</Text>
                  </Button>
                );
              })}
            ...
            <Stack minW="0" spacing="12px">
              <Box borderRadius="22px" bg="navy.800" color="white" p="16px" boxShadow="lg">
              ...
              {selectedBusiness && (
                <BusinessCard
                  ...
                  contactsOpen={openContactId === selectedBusiness.id}
                  favorite={favoriteIds.includes(selectedBusiness.id)}
                  muted={muted}
                  panelBg={panelBg}
                  onOpenContacts={() => setOpenContactId((current) => current === selectedBusiness.id ? '' : selectedBusiness.id)}
...
function BusinessCard({ business, contactsOpen, favorite, muted, panelBg, onSelect, onOpenContacts, onFavorite }) {
  ...
  return (
    <Box onClick={onSelect} p={{ base: '14px', md: '16px' }} minW="0" minH="224px" borderRadius="24px" bg={panelBg} border="1px solid" borderColor="gray.100" boxShadow="md" position="relative" overflow="visible" transition="all .2s ease" _hover={{ transform: 'translateY(-5px)', boxShadow: 'xl', borderColor: 'brand.300' }}>
      ...
      {contactsOpen && (
        <Box position="absolute" zIndex={8} left={{ base: '10px', md: '18px' }} right={{ base: '10px', md: '18px' }} bottom="-18px" p="12px" borderRadius="22px" bg={panelBg} border="1px solid" borderColor="brand.200" boxShadow="0 18px 40px rgba(15, 23, 42, .28)" animation={`${floatIn} .18s ease-out`}>
          <SimpleGrid columns={Math.min(contactLinks.length, 4)} spacing="8px">
            {contactLinks.map((businessLink) => {
              ...
              return (
                <Stack key={businessLink} as="a" href={hrefFor(businessLink)} target="_blank" rel="noopener noreferrer" align="center" spacing="4px" minW="0" color="inherit" onClick={(event) => event.stopPropagation()} _hover={{ textDecoration: 'none', transform: 'translateY(-3px)' }} transition="transform .18s ease">
                  <Flex align="center" justify="center" w={{ base: '46px', md: '54px' }} h={{ base: '46px', md: '54px' }} borderRadius="full" bg={meta.bg} color="white" boxShadow="0 10px 18px rgba(15, 23, 42, .22)"><Icon as={meta.icon} w={{ base: '19px', md: '22px' }} h={{ base: '19px', md: '22px' }} /></Flex>
