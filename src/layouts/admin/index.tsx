...
export default function Dashboard(props: { [x: string]: any }) {
  ...
  const getRoutes = (routes: RoutesType[]): JSX.Element[] => {
    const validLayouts = ['/order', '/purchase-order', '/change-order', '/invoice', '/receipt', '/expense', '/pay-roll', '/material-request', '/reimbursement', '/service', '/dashboard'];
  ...
  const shouldShowBox = currentPath.includes('/order/index') ||  currentPath.includes('/dashboard/index');
				...
				<Sidebar routes={routes} display='none' {...rest} />
					...
					>
						...
						<Box mx='auto' p={{ base: '20px', md: '30px' }} pe='20px' minH='100vh' pt='50px'>						
							<Switch>
								{getRoutes(routes)}
								<Redirect from='/' to={`/order/index/${search}`} />
							</Switch>
						</Box>
					</Box>
